  ...
  //helper funciton to get the time value for each server
  let jsonDouble = (json) => {
    return (json["doubleValue"]).toFixed(2)
  }
  //helper funciton to get the time value for each server
  let jsonString = (json) => {
  ...
  fetch('https://firestore.googleapis.com/v1/projects/subsaverdotspace/databases/(default)/documents/user_data/dylan/')
    ...
    .then((json) => {
      ...
      let HBO = jsonDouble(data["time_hbo"]);
      let Hulu = jsonDouble(data["time_hulu"]);
      let Netflix = jsonDouble(data["time_netflix"]);
      let Youtube = jsonDouble(data["time_youtube"]);
      let Xfinity = jsonDouble(data["time_xfinity"]);
      let Crunchyroll = jsonDouble(data["time_crunchyroll"]);
      let DisneyPlus = jsonDouble(data["time_disneyplus"]);
      let AppleTV = jsonDouble(data["time_appletv"]);
      let AmazonPrime = jsonDouble(data["time_amazonprime"]);
      //finds global values 
      let GHBO = jsonDouble(data["global_hbo"]);
      let GHulu = jsonDouble(data["global_hulu"]);
      let GNetflix = jsonDouble(data["global_netflix"]);
      let GYoutube = jsonDouble(data["global_youtube"]);
      let GXfinity = jsonDouble(data["global_xfinity"]);
      let GCrunchyroll = jsonDouble(data["global_crunchyroll"]);
      let GDisneyPlus = jsonDouble(data["global_disneyplus"]);
      let GAppleTV = jsonDouble(data["global_appletv"]);
      let GAmazonPrime = jsonDouble(data["global_amazonprime"]);
